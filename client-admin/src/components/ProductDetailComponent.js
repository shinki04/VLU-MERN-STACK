...
class ProductDetail extends Component {
  ...
  render() {
    const cates = this.state.categories.map((cate) => {
      if (this.props.item != null && this.props.item.category) {
        return (
          <option
            key={cate._id}
            value={cate._id}
  ...
  componentDidUpdate(prevProps) {
    if (this.props.item !== prevProps.item) {
      this.setState({
        ...
        txtPrice: this.props.item ? this.props.item.price : 0,
        cmbCategory: this.props.item && this.props.item.category ? this.props.item.category._id : "",
        imgProduct: this.props.item ? "data:image/jpg;base64," + this.props.item.image : ""
      });
    }
  }
